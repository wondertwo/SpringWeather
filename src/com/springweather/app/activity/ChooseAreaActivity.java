...
import com.springweather.app.R;
...
import com.springweather.app.util.HttpCallbackListener;
...
public class ChooseAreaActivity extends Activity {
	...
	private SpringWeatherDB springWeatherDB;
	...
	private void queryProvinces() {
		provinceList = springWeatherDB.loadProvinces();
	...
	/**
	 * 查询选中省的所有市，优先从数据库查询，如果没有查到再去服务器上查询
	 */
	private void queryCities() {
		cityList = springWeatherDB.loadCities(selectedProvince.getId());
		if (cityList.size() > 0) {
			dataList.clear();
	...
	/**
	 * 查询选中市的所有县，优先从数据库查询，如果没有查到再去服务器上查询
	 */
	private void queryCounties() {
		countyList = springWeatherDB.loadCounties(selectedCity.getId());
	...
	private void showProgressDialog() {
		if (progressDialog == null) {
			...
			progressDialog.setMessage("正在加载(^o^)");
	...
	/**
	 * 捕获Back按键，根据当前的级别来判断，此时应该返回市列表、省列表，还是 直接退出
	 */
	@Override
